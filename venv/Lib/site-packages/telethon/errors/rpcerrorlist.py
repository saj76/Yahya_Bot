...
class FloodTestPhoneWaitError(FloodError):
    def __init__(self, request, **kwargs):
        ...
        super(Exception, self).__init__('A wait of {} seconds is required in the test servers'.format(self.seconds) + self._fmt_request(request))


class FloodWaitError(FloodError):
    def __init__(self, request, **kwargs):
        self.seconds = int(kwargs.get('capture', 0))
        super(Exception, self).__init__('A wait of {} seconds is required'.format(self.seconds) + self._fmt_request(request))


class TakeoutInitDelayError(FloodError):
    def __init__(self, request, **kwargs):
        self.seconds = int(kwargs.get('capture', 0))
        super(Exception, self).__init__('A wait of {} seconds is required before being able to initiate the takeout'.format(self.seconds) + self._fmt_request(request))
...
class FileMigrateError(InvalidDCError):
    def __init__(self, request, **kwargs):
        ...
        super(Exception, self).__init__('The file to be accessed is currently stored in DC {}'.format(self.new_dc) + self._fmt_request(request))


class NetworkMigrateError(InvalidDCError):
    def __init__(self, request, **kwargs):
        self.new_dc = int(kwargs.get('capture', 0))
        super(Exception, self).__init__('The source IP address is associated with DC {}'.format(self.new_dc) + self._fmt_request(request))


class PhoneMigrateError(InvalidDCError):
    def __init__(self, request, **kwargs):
        self.new_dc = int(kwargs.get('capture', 0))
        super(Exception, self).__init__('The phone number a user is trying to use for authorization is associated with DC {}'.format(self.new_dc) + self._fmt_request(request))


class UserMigrateError(InvalidDCError):
    def __init__(self, request, **kwargs):
        self.new_dc = int(kwargs.get('capture', 0))
        super(Exception, self).__init__('The user whose identity is being used to execute queries is associated with DC {}'.format(self.new_dc) + self._fmt_request(request))


class TimeoutError(RPCErrorNeg503):
    def __init__(self, request, **kwargs):
        super(Exception, self).__init__('No description known' + self._fmt_request(request))
...
class ChatAdminRequiredError(BadRequestError):
    def __init__(self, request, **kwargs):
        super(Exception, self).__init__('Chat admin privileges are required to do that in the specified chat (for example, to send a message in a channel which is not yours)' + self._fmt_request(request))
...
class EmailUnconfirmedError(BadRequestError):
    def __init__(self, request, **kwargs):
        super(Exception, self).__init__('Email unconfirmed' + self._fmt_request(request))
...
class FilePartMissingError(BadRequestError):
    def __init__(self, request, **kwargs):
        ...
        super(Exception, self).__init__('Part {} of the file is missing from storage'.format(self.which) + self._fmt_request(request))
...
class InterdcCallErrorError(BadRequestError):
    def __init__(self, request, **kwargs):
        ...
        super(Exception, self).__init__('No description known'.format(self.x) + self._fmt_request(request))
